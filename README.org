...
- Features
  ...
  - Optional inclusion of the surrounding Org entry as context (=:entry t=)
...
The =:entry t= header argument prepends the prose preceding the block —
from the start of the enclosing heading's body (after =:PROPERTIES:=,
planning, and =:LOGBOOK:= drawers) up to the block itself — to the body
that is sent. This is useful for asking the model to summarize,
critique, or transform notes you've already written:
...
still build the prior conversational turns, while =:entry= augments

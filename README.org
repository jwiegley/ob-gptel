...
2. /Caching/: Add =:cache yes= to avoid re-running expensive queries during
   export.

3. /Templates/: Create template functions that generate gptel blocks with
...
** session and system per file headers

these don't work because of how babel parameters are collected

#+begin_src org
#+property: header-args:gptel+ :system "system prompt"
#+property: header-args:gptel+ :session test
#+end_src
...
#+begin_src gptel :prompt math-context :dry-run t :wrap src elisp
Can you give me an example calculation?
#+end_src
...
#+begin_src gptel :system "You are a creative writing assistant." :wrap src elisp :dry-run t
Help me start a short story about a time traveler.
#+end_src
...
(gptel-make-preset 'test
  :description "Preset for org mode"
  :backend "ChatGPT"
...
#+begin_src gptel :session foo :preset test :dry-run t :wrap src elisp
Continue the story with an unexpected twist.
#+end_src
...
#+begin_src emacs-lisp
(append gptel-context (split-string "test.png"))
(setq gptel-use-context 'user)
(setq gptel-context '("./test.png" "./test.svg"))
(setq gptel-context '("test.org"))
...
#+begin_src gptel :dry-run t :wrap src elisp :media t :context test.txt
[[file:test.png]]
...
#+begin_src gptel :dry-run t :wrap src elisp :tools read_buffer
what's here?
#+end_src

#+RESULTS:
#+begin_src elisp
(:model "gpt-4o-mini" :messages [(:role "user" :content "what's here?")] :stream
        :json-false :temperature 1.0 :tools
        [(:type "function" :function
                (:name "read_buffer" :description
                       "return the contents of an emacs buffer" :parameters
                       (:type "object" :properties
                              (:buffer
                               (:type "string" :description
                                      "the name of the buffer whose contents are to be retrieved"))
                              :required ["buffer"] :additionalProperties
                              :json-false)))]
